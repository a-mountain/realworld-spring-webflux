...
public class ArticleController {
    ...
    public Mono<ArticleViewWrapper> getArticle(@PathVariable String slug) {
        return userContextProvider.getCurrentUserOrEmpty()
                ...
                .switchIfEmpty(articleFacade.getArticle(slug, Optional.empty())).map(ArticleViewWrapper::new);
    }
    ...
    public Mono<ArticleViewWrapper> updateArticle(@RequestBody UpdateArticleRequestWrapper request, @PathVariable String slug) {
        return userContextProvider.getCurrentUserOrEmpty()
                .flatMap(currentUser -> articleFacade.updateArticle(slug, request.getContent(), currentUser)).map(ArticleViewWrapper::new);
    }
    ...
    public Mono<CommentViewWrapper> addComment(@PathVariable String slug, @RequestBody CreateCommentRequestWrapper request) {
        return userContextProvider.getCurrentUserOrEmpty()
                .flatMap(currentUser -> articleFacade.addComment(slug, request.getContent(), currentUser)).map(CommentViewWrapper::new);
    }
    ...
    public Mono<ArticleViewWrapper> favoriteArticle(@PathVariable String slug) {
        return userContextProvider.getCurrentUserOrEmpty()
                .flatMap(currentUser -> articleFacade.favoriteArticle(slug, currentUser)).map(ArticleViewWrapper::new);
    }
    ...
    public Mono<ArticleViewWrapper> unfavoriteArticle(@PathVariable String slug) {
        return userContextProvider.getCurrentUserOrEmpty()
                .flatMap(currentUser -> articleFacade.unfavoriteArticle(slug, currentUser)).map(ArticleViewWrapper::new);
    }
